defmodule CloudWatch do
  ...
  defp do_flush(state, opts, log_group_name, log_stream_name) do
    events = %{
      logEvents: Enum.sort_by(state.buffer, &(&1.timestamp)),
      ...
      {:error, {"ThrottlingException", "Rate exceeded"}} ->
        ...
        if state.purge_buffer_if_throttled do
          ...
          state = state
                  |> add_message(
                       :error,
                       "CloudWatch Log ThrottlingException: delaying transfer}",
          ...
          |> add_message(
               :error,
               "CloudWatch Log ThrottlingException: #{inspect lost_msg_count} messages were lost!}",
               Logger.Utils.timestamp(utc_log?),
               Logger.metadata()
             )
        end
